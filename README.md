...
│       ├── engine/                 # 创建引擎：容量报告轮询、错误分类、退避、可用区选择
...
2. 进入 **“抢机调度 (Launcher)”** 页面：
   ...
   - **排队策略**：容量不足时加入排队：ARM A1 每 3–5 分钟读取一次官方容量报告（只读接口），报告显示有容量才真正创建；AMD E2.1.Micro 不适用容量报告，按同样间隔直接尝试；默认最多持续 7 天、每天不超过 30 次真实创建，429 限流时指数退避。
